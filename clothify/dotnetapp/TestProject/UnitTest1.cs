...
[Test, Order(5)]
public async Task Backend_TestAddProductAdmin()
{
    string uniqueId = Guid.NewGuid().ToString();
    ...
    // Deserialize the response as a JObject
    string responseBody = await loginResponse.Content.ReadAsStringAsync();
    dynamic responseMap = JsonConvert.DeserializeObject(responseBody);

    // Replace 'Token' with the correct property name for the token in your JSON response
    string token = responseMap.token.token;

    Assert.IsNotNull(token);

    string uniqueTitle = Guid.NewGuid().ToString();
    string uniqueProductTitle = $"productTitle_{uniqueTitle}";

    string productJson = $"{{\"ProductType\":\"{uniqueProductTitle}\",\"ProductImageUrl\":\"test\",\"ProductDetails\":\"test\",\"ProductPrice\":10,\"Quantity\":1}}";
    _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
    HttpResponseMessage response = await _httpClient.PostAsync("/api/product",
    ...
    [Test, Order(6)]
    public async Task Backend_TestGetAllProductsForAdminAndCustomer()
    {
       string uniqueId = Guid.NewGuid().ToString();
    ...
    public async Task Backend_TestPostReview()
    {
        ...
        string customerAuthToken = responseMap.token.token;
    ...
    public async Task Backend_TestGetAllReviews()
    {
        ...
        string responseString = await loginResponse.Content.ReadAsStringAsync();
        dynamic responseMap = JsonConvert.DeserializeObject(responseString);
        string adminAuthToken = responseMap.token.token;
...
public async Task Backend_TestDeleteReview()
{
    ...
    string customerAuthToken = loginResponseMap.token.token;
...
public async Task Backend_TestGetAllOrders()
{
    ...
    HttpResponseMessage getOrdersResponse = await _httpClient.GetAsync("/api/order");
